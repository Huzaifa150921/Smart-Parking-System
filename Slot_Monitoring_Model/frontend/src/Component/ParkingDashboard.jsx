...
function ParkingCards({ requests, onDetailClick }) {
        ...
        >
          ...
          <Chip
            ...
            sx={{
              ...
              mb: 2,
              borderRadius: 2,
              background: item.status === "rejected" ? "#f44336" : "#ff9800",
            }}
          />
          ...
          <Box mt={1} width="100%">
            <Button
              ...
              onClick={() => onDetailClick(item.uid)}
            >
              View Details
            </Button>
          </Box>
...
function ParkingDashboard() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      collection(firestore, "pending_parking"),
      async (snap) => {
        const data = await Promise.all(
          snap.docs.map(async (d) => {
            ...
            if (info.uid) {
              try {
                const ref = doc(firestore, "owners", info.uid);
                const snap = await getDoc(ref);
                if (snap.exists()) {
                  firstName = snap.data().firstName || firstName;
                }
              } catch (e) {
                console.error("Error fetching owner:", e);
              }
            }
            return { id: d.id, ...info, firstName };
          })
        );
        setRequests(data);
      }
  ...
  const filtered = (status) =>
    requests.filter(
      (r) => r.status?.toLowerCase() === status && r.isFormSubmit
    );
    ...
    >
      ...
      >
        <Toolbar>
          <Box sx={{ display: "flex", alignItems: "center", flexGrow: 1 }}>
            ...
            >
              <FaParking size={24} style={{ marginRight: 8 }} />
              Parking Dashboard
            </Box>
          </Box>
      ...
      >
        <Grid container spacing={3}>
          <ParkingCards
            requests={value === 0 ? filtered("pending") : filtered("rejected")}
            onDetailClick={(uid) => navigate(`/parking/${uid}`)}
          />
        </Grid>
      </Box>
...
export default function ThemedParkingDashboard() {
