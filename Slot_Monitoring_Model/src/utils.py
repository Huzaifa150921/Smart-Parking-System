...
class Park_classifier():
    def classify_status(self, prosessed_image:np.ndarray, threshold:int=900):
        """
        Returns a list of booleans for each slot: True=free, False=occupied.
        """
        status_list = []
        for x, y in self.car_park_positions:
            ...
            status_list.append(count < threshold)
        return status_list
